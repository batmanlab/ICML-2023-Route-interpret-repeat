...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, pretrained="n"):
    ...
    def forward(self, embedding):
        ...
        if self.hybrid:
            x = self.hybrid_model(embedding)
        if self.pretrained == "n":
            x = embedding
        elif self.pretrained == "y":
            x = self.patch_embeddings(embedding)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, pretrained):
        super(Transformer, self).__init__()
        self.embeddings = Embeddings(config, img_size=img_size, pretrained=pretrained)
        self.encoder = Encoder(config)

    def forward(self, embedding):
        embedding_output = self.embeddings(embedding)
        part_encoded = self.encoder(embedding_output)
        return part_encoded


class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, pretrained="n"):
        super(VisionTransformer, self).__init__()
        self.num_classes = num_classes
        self.smoothing_value = smoothing_value
        self.zero_head = zero_head
        self.classifier = config.classifier
        self.transformer = Transformer(config, img_size, pretrained)
        self.part_head = Linear(config.hidden_size, num_classes)

    def forward(self, embedding, labels=None):
        part_tokens = self.transformer(embedding)
...
CONFIGS = {
    'ViT-B_32_densenet': VIT_configs.get_b32_config_densenet(),
    'ViT-B_16_densenet': VIT_configs.get_b16_config_densenet(),

...
from BB.models import VIT_configs
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, grad_cam=False):
    ...
    def forward(self, input_ids):
        ...
        part_encoded, attn_weights = self.encoder(embedding_output)
        if self.grad_cam:
            return part_encoded, attn_weights, embedding_output
        else:
            return part_encoded, attn_weights


class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, grad_cam=False):
        ...
        self.transformer = Transformer(config, img_size, grad_cam)
        self.part_head = Linear(config.hidden_size, num_classes)

    def forward(self, x, labels=None):
        if self.grad_cam:
            part_tokens, attn_weights, embedding_output = self.transformer(x)
        else:
            part_tokens, attn_weights = self.transformer(x)
        ...
        if labels is not None:
            ...
            return part_logits, part_tokens, attn_weights, embedding_output
        else:
...
class VisionTransformer_baseline(nn.Module):
    def __init__(
            self, config, img_size=224, num_classes=21843, op_size=108, smoothing_value=0, zero_head=False, test=False
    ):
        super(VisionTransformer_baseline, self).__init__()
        self.num_classes = num_classes
        self.smoothing_value = smoothing_value
        self.zero_head = zero_head
        self.classifier = config.classifier
        ...
        self.part_head = Linear(config.hidden_size, num_classes)
        self.t = torch.nn.Linear(in_features=768, out_features=op_size, bias=True)
        self.test = test

    def forward(self, x):
        part_tokens, attn_weights = self.transformer(x)
        if self.test:
            return self.t(part_tokens[:, 0]), part_tokens[:, 0]
        else:
            return self.t(part_tokens[:, 0])
...
if __name__ == "__main__":
    ...
    gt = torch.ones(10, dtype=torch.int64)

    _part_logits, _part_tokens = model(x)
    print(_part_logits.size())
    ...
    # # )
    # print("Saved model getting loaded")
    # model = VisionTransformer(
    #     _config, _img_size, zero_head=True, num_classes=_num_classes, smoothing_value=_smoothing_value
    # )
    #
    # model.load_state_dict(torch.load(os.path.join(out_folder, f"g_best_model_epoch_10.pth.tar"))["model"])
    # print("model loaded")

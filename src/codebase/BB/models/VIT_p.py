...
class VisionTransformer_projected(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, smoothing_value=0, zero_head=False, get_phi=False):
        super(VisionTransformer_projected, self).__init__()
    ...
    def forward(self, x, concept, scale, sigma, device, labels=None):
        part_tokens = self.transformer(x)
        # backbone = part_tokens[:, 0]
        I = torch.eye(part_tokens.size(0)).to(device)
        xc = concept.to(torch.float)
        xc_T = torch.transpose(concept, 0, 1)
        # print(sigma.dtype)
        # print(xc_T.dtype)
        # print(scale)
        # print(backbone.dtype)
        phi_r_x = torch.mm(
            (I - scale * torch.mm(xc, torch.mm(sigma, xc_T))),
            part_tokens.view(part_tokens.size(0), -1)).view(
            part_tokens.size(0), part_tokens.size(1), part_tokens.size(2)
        )
        part_logits = self.part_head(phi_r_x[:, 0])
...
class VisionTransformer_baseline(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, op_size=108, smoothing_value=0, zero_head=False):
